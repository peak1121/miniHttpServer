#ifndef HTTPSERVER_NET_EVENTLOOP_H
#define HTTPSERVER_NET_EVENTLOOP_H
...
private:
    ...
    bool looping_;//标记loop是否正在循环
    std::atomic<bool> quit_;//标记是否退出
    bool eventHandling_; //处理任务期间为true
    bool callingPendingFunctors_; //执行挂起任务期间为true
    int64_t iteration_;
    ...
    ChannelList activeChannels_;//保存epoll_wait返回的事件对应channel
